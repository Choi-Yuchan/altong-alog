...
const NoticeReply_Contents = ({reply, selected, onRemove}) => {

    const [show, setShow] = useState(true);

    return(
            <Contents to="/" onClick={()=>{onRemove(selected)}}>{/* 클릭 시 해당 컨텐츠로 이동 */}
                <LogoDiv><Logo src={ reply.state ? process.env.PUBLIC_URL + '/images/logo_alog.png' : process.env.PUBLIC_URL + '/images/logo_dalog.png'} /></LogoDiv>
                    <Sentences>
                       ...
                       <SubSentences>{reply.word}</SubSentences>
                       </Sentences>
                    <LanDiv>
...
export default NoticeReply_Contents;
