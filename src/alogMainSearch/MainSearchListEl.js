...
import FormatDateAsText from '../components/function/FormatDateAsText';
...
function SearchListEl2(props) {
    ...
    return (
        <ListBox imgurl={process.env.PUBLIC_URL + '/images/' + props.result.backImg}>
            ...
            <ListContain>
                <WhiteBack></WhiteBack>
                <ListText>{props.result.title}</ListText>
                <ListProfile imgurl={process.env.PUBLIC_URL + '/images/' + props.result.profile}></ListProfile>
                <ListBy>by {props.result.nickname}</ListBy>
            </ListContain>
            <ListActive>
                <ListTime>{FormatDateAsText(props.result.time)}</ListTime>
                <ListCheck><ListCheckIcon src={process.env.PUBLIC_URL + '/images/icon_view.svg'}></ListCheckIcon> {props.result.view}</ListCheck>
